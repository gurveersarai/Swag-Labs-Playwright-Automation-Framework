...
test.only('should be able to see the "Checkout: Your Information" title on the checkout form page', async ({ firstCheckoutFormPage, poManager }) => {
    const pageTitle = await poManager.commonElements.pageTitle.textContent();
    expect(pageTitle).toBe("Checkout: Your Information");
});

test.only('should be able to see a checkout form displayed', async({ firstCheckoutFormPage, poManager }) => {
    await expect(poManager.checkoutFormPage.checkoutFormContainer).toBeVisible();
});

test.only('should be redirected to the cart page when clicking cancel button on checkout form page', async ({ firstCheckoutFormPage, poManager }) => {
...
test.only('should display error message when trying to continue with empty checkout form', async ({ firstCheckoutFormPage, poManager }) => {
...
test.only('should display error message when trying to continue with partially filled checkout form', async ({ firstCheckoutFormPage, poManager }) => {
...
test.only('should be able to proceed to the next checkout step with valid checkout form', async ({ firstCheckoutFormPage, poManager }) => {
